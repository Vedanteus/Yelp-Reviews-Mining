...
Original file is located at
    https://colab.research.google.com/drive/1Z5CC9CPg6DVGEPd30ap8OIuJKDqiOwdD
"""
...
df = pd.read_csv("/content/drive/MyDrive/Colab Notebooks/Data/Yelp Restaurant Reviews.csv")
...
# Converting ratings to 1 and 0
df['Rating'] = df['Rating'].map({4:1,5:1,1:0,2:0,3:0})
df.info()
...
df['cleaned_review'] = df['Review Text'].apply(cleaner)
...
pd.DataFrame(pd.Series(tfidf.get_feature_names_out())).to_csv('vocabulary_yelpreviews.csv', header=False, index=False)
...
joblib.dump(clf, 'yelpreviews_model.sav')